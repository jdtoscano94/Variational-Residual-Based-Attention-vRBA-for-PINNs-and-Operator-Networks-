...
for epoch in range(num_epochs):
    ...
    for _ in range(0, X_func_train.shape[0] // train_batch_size):
            ...
            lambda_scores = Lambda_tensor.sum(dim=1)
            ...
            lambda_it=Par['phi']*q_it/torch.amax(q_it)+(1-Par['phi'])
            q_pdf=lambda_it/lambda_it.sum()
            
            # Sample a batch of indices based on the distribution
