...
class CustomLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true, Par, Lambda=None):
        ...
        if Lambda is not None:
            ...
            mu_r=Par['c_log']*torch.amax(r_i,dim=(1,2,3),keepdims=True)/log_it
            beta_it=1/(mu_r+1e-16)
            q_it = torch.exp(beta_it * r_i)
            lambda_it=Par['phi']*q_it/torch.amax(q_it,dim=(1,2,3),keepdims=True)+(1-Par['phi'])
...
for epoch in range(num_epochs):
    ...
    for _ in range(0, x_train.shape[0] // train_batch_size):
            ...
            lambda_scores = Lambda_tensor.sum(dim=(1, 2, 3))
            # Normalize to create the probability distribution
            log_it = torch.log(torch.tensor(Par['it'] + 2.0))
            mu_r=Par['c_log']*torch.amax(lambda_scores)/log_it
            beta_it=1/(mu_r+1e-16)
            q_it = torch.exp(beta_it * lambda_scores)
            q_pdf=Par['phi']*q_it/torch.amax(q_it)+(1-Par['phi'])
            q_pdf=q_pdf/q_pdf.sum()

            # 2. Sample a batch of indices based on the distribution q_pdf
            idx_train = np.random.choice(
                a=x_train.shape[0],          # Sample from all possible indices (0 to 799)
                size=train_batch_size,       # Get a batch of indices
                replace=False,               # Don't pick the same sample twice in one batch
                p=q_pdf.cpu().numpy()        # Use the calculated probabilities
            )
    ...
    print(time_stamp + f' - Epoch {epoch + 1}/{num_epochs},it:{total_iterations} Train Loss: {train_loss:.4e}, Val Loss: {val_loss:.4e}, best model: {best_model_id}, LR: {scheduler.get_last_lr()[0]:.4e}, epoch time: {elapsed_time:.2f}, snr: {snr:.4e}, var(R): {var_residual:.4e}, var(L*R): {L_var_residual:.4e}, WD: {wd:.4e}, max_RBA:{max_RBA:.4e}')
