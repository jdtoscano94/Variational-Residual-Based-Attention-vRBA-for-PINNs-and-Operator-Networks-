...
for epoch in range(num_epochs):
    ...
    # Training
    with torch.no_grad():
        # sample_weights is on the CPU
        sample_weights = Lambda_tensor.sum(dim=(1, 2, 3))

    # By removing 'device=device', log_it and all subsequent tensors will be on the CPU
    log_it = torch.log(torch.tensor(Par['it'] + 2.0))
    mu_r = Par['c_log'] * torch.amax(sample_weights) / log_it
    beta_it = 1 / (mu_r + 1e-16)
    q_pdf = torch.exp(beta_it * sample_weights)
    
    # Final sample_weights for the sampler are now correctly calculated on the CPU
    sample_weights_final = Par['phi'] * q_pdf / torch.amax(q_pdf) + (1 - Par['phi'])
    
    # Create the weighted sampler for this training phase
    sampler = WeightedRandomSampler(sample_weights_final, num_samples=len(sample_weights_final), replacement=True)

    # Create the special DataLoader for this epoch's training phase
